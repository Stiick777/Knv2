...
let handler = async (message, { conn, text, usedPrefix, command }) => {
  ...
  // 🔥 STREAMING: generar video sin descargar en disco
  async function createVideoMessage(url) {
    // Descargar video como buffer sin usar archivos temporales
    const { data } = await axios.get(url, { responseType: "arraybuffer" });
    const buffer = Buffer.from(data);

    // Crear mensaje de video usando buffer
    const { videoMessage } = await generateWAMessageContent(
  ...
  try {
    ...
    // 🔥 Generar las tarjetas del carrusel con videos en streaming
    for (let result of topResults) {
      results.push({
        body: proto.Message.InteractiveMessage.Body.fromObject({ text: null }),
        footer: proto.Message.InteractiveMessage.Footer.fromObject({ text: titulowm }),
