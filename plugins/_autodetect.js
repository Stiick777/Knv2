...
let pp = await conn.profilePictureUrl(conn.user.jid).catch(_ => imagen3)
...
// Bienvenida (Incluyendo tipo 27)
  /*if (chat.welcome && (m.messageStubType === WAMessageStubType.GROUP_PARTICIPANT_ADD || m.messageStubType === 27)) {
  ...
  // Despedida (Incluyendo tipos 28 y 32)
  if (chat.welcome && (m.messageStubType === WAMessageStubType.GROUP_PARTICIPANT_REMOVE || m.messageStubType === WAMessageStubType.GROUP_PARTICIPANT_LEAVE || m.messageStubType === 28 || m.messageStubType === 32)) {
    ...
    await conn.sendMessage(m.chat, { 
      ...
      contextInfo: { 
        ...
        "externalAdReply": { 
          "title": "Despedida", 
          "body": `${userName} ha dejado el grupo.`,
          "thumbnail": imagen3,
          "sourceUrl": rcanal,
          "showAdAttribution": true 
        } 
...
await this.sendMessage(m.chat, { text: `⚠️ ${usuario} *Ha cambiado el nombre del grupo*`, mentions: [m.sender], mentions: (await conn.groupMetadata(m.chat)).participants.map(v => v.id) }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100}) 
